...
.MODIFICATIONS
    This file has been modified by Fiskum IT as part of the "Fiskum IT CoreCycler
    Manager" wrapper project. The original engine logic, license and authorship
    above are unchanged - the modifications below are additive:
      - An always-on core/offset snapshot file (written/read/removed throughout
        this script) that lets the Manager detect and resume an in-progress
        Curve Optimizer / voltage offset search after a crash or unexpected exit.
      - A "Decreasing" search direction for the automatic voltage offset
        adjustment (in addition to the original "Increasing" direction), with a
...
$Host.UI.RawUI.WindowTitle = ('CoreCycler ' + $version + ' starting - tilpasset av Fiskum IT')



Write-Host('Starting CoreCycler v' + $version + '...')
Write-Host('Tilpasset av Fiskum IT (basert pa CoreCycler av sp00n, https://github.com/sp00n/corecycler)') -ForegroundColor Cyan
...
# Fiskum IT addition: an always-on snapshot of the active core + the full per-core offset array
# Unlike the .automode file above, this is written whenever Automatic Test Mode is enabled at all
# (not just when enableResumeAfterUnexpectedExit is set), so the FiskumIT-CoreCyclerManager UI can
# show live offsets and resume a test exactly where it left off after a crash or manual stop
$coreOffsetSnapshotFile                  = $logFilePathAbsolute + 'fiskumit-offset-snapshot.json'
$coreOffsetSnapshotFileTemp              = $logFilePathAbsolute + 'fiskumit-offset-snapshot-temp.json'
...
# Fiskum IT addition: the search direction for the automatic adjustment
# "Increasing" (default) is the original behavior: start at "startValues" and only ever INCREASE the
#   Curve Optimizer / voltage offset (i.e. make it less aggressive) when a core throws an error
# "Decreasing" is used by the "Assistert undervolting" workflow in the FiskumIT-CoreCyclerManager:
#   start at "startValues" (typically 0) and make the value MORE aggressive (more negative) every time
#   the currently tested core passes without an error, repeating the same core
#   Once that core throws an error, the value is stepped back up by one "incrementBy" (as with
#   "Increasing") and, if "repeatCoreOnError" is 0, locked there before moving on to the next core
#   See configs/Ryzen.AutomaticTestMode.Start.ini for the original sp00n example using "Increasing",
#   or Manager/config/AssistedUndervolting_Ryzen.ini for a ready-to-use "Decreasing" example
#
# Default: Increasing
...
# startValues = -120
#
# Fiskum IT addition: regardless of what this setting says, if a valid offset snapshot
# file from a previous Automatic Test Mode run exists (CoreCycler/logs/fiskumit-offset-
# snapshot.json - written whenever the active core or a value changes, for as long as
# Automatic Test Mode is enabled at all), its per-core values are used instead. This is
# what lets a crash/stop/exit and then a resume - or moving on to the next test in a
# sequence of several test profiles - correctly carry every core's already-discovered
# value forward, instead of re-reading the live CPU state (which, with
# setVoltageOnlyForTestedCore enabled below, only ever has ONE core's real value applied
# on the silicon at any given moment - every other core reads back as 0 there). The
# snapshot is only ignored if it is missing, unreadable, or its core count no longer
...
# Fiskum IT addition: the lower SAFETY LIMIT for "searchDirection = Decreasing" (see above)
# Only used when searchDirection is "Decreasing" - ignored for the default "Increasing" search
# Prevents the automatic search from pushing a core's Curve Optimizer value past the CPU's actual
# supported range even if the stress test never reports an error at extreme values (e.g. silent
# data corruption instead of a crash/WHEA event)
# -30 is the documented minimum Curve Optimizer value for Ryzen 5000, see "startValues" above -
# adjust if your specific CPU/BIOS allows for a different range
#
# Default: -30
minValue = -30
...
<#
.DESCRIPTION
    Fiskum IT addition: write the always-on core/offset snapshot file
    Unlike Set-AutoModeFile, this does not depend on enableResumeAfterUnexpectedExit, only on
    Automatic Test Mode being enabled at all, since the FiskumIT-CoreCyclerManager UI relies on
    this file for both the live offset display and crash/stop/exit resume, for every test profile
.PARAMETER ActiveCore
    [Int] The core that is currently active / was last active
.OUTPUTS
    [Void]
#>
function Set-CoreOffsetSnapshot {
    param(
        [Parameter(Mandatory=$true)] [Int] $ActiveCore
    )
    ...
    try {
        $lockedValues = [Ordered] @{}

        foreach ($core in @($Script:coresWithError)) {
            $lockedValues[[String] $core] = [Int] $voltageCurrentValues[$core]
        }

        $snapshotObject = [Ordered] @{
            'timestamp'       = [UInt64] (Get-Date -UFormat %s -Millisecond 0)
            'activeCore'      = $ActiveCore
            'searchDirection' = $(if ($settings.AutomaticTestMode.searchDirection) { [String] $settings.AutomaticTestMode.searchDirection } else { 'Increasing' })
            'incrementBy'     = [Int] $settings['AutomaticTestMode']['incrementBy']
            'maxValue'        = [Int] $settings['AutomaticTestMode']['maxValue']
            'offsets'         = @($voltageCurrentValues)
            'lockedCores'     = $lockedValues
        }

        $snapshotJson = ConvertTo-Json $snapshotObject

        # Fiskum IT fix: Rename-Item -Force does NOT overwrite an existing destination file
        # (the -Force there only bypasses read-only/hidden attributes) - the old file from a
        # previous write must be removed first, exactly as Set-AutoModeFile does above
        if (Test-Path -LiteralPath $coreOffsetSnapshotFile -PathType Leaf) {
            $null = Remove-Item -LiteralPath $coreOffsetSnapshotFile -Force
        }

        $null = New-Item $coreOffsetSnapshotFileTemp -ItemType File -Force

        [System.IO.File]::WriteAllLines($coreOffsetSnapshotFileTemp, $snapshotJson)

        if ($canUseFlushToDisk) {
            Save-CachedDataToDisk
        }

        $null = Rename-Item -LiteralPath $coreOffsetSnapshotFileTemp -NewName $coreOffsetSnapshotFile -Force
    }
    catch {
        Write-DebugText('Could not write the Fiskum IT offset snapshot file: ' + $_)
    }
}
...
<#
.DESCRIPTION
    Fiskum IT addition: remove the always-on core/offset snapshot file
    Called on a clean exit, alongside Remove-AutoModeFile, so that the Manager can tell a clean
    shutdown apart from a crash (the file is only left behind after an unexpected interruption)
.OUTPUTS
    [Void]
#>
function Remove-CoreOffsetSnapshot {
    if (Test-Path -LiteralPath $coreOffsetSnapshotFile -PathType Leaf) {
        Remove-Item -LiteralPath $coreOffsetSnapshotFile -Force
    }

    if (Test-Path -LiteralPath $coreOffsetSnapshotFileTemp -PathType Leaf) {
        Remove-Item -LiteralPath $coreOffsetSnapshotFileTemp -Force
    }
}
...
<#
.DESCRIPTION
    Fiskum IT addition: read the per-core values out of the always-on offset snapshot file
    left behind by a previous Automatic Test Mode run (this same test, the previous test in a
    Vanlig stabilitetstest sequence, or Assistert undervolting just before it), if one exists
    and matches the current core count.

    This exists because setVoltageOnlyForTestedCore/voltageValueForNotTestedCores means only the
    ACTIVELY tested core ever has a non-zero Curve Optimizer value applied on the silicon at any
    given moment - every other core is held at 0 there. Reading "CurrentValues" straight off the
    CPU at startup would therefore only ever see ONE core's real value and 0 for all the others,
    silently discarding every previously discovered value. The snapshot file instead tracks the
    full, correct per-core array regardless of what is momentarily on the silicon.
.PARAMETER ExpectedCoreCount
    [Int] The number of physical cores the array must contain to be considered usable
.OUTPUTS
    [Array] The per-core values, or $null if no usable snapshot was found
#>
function Get-FiskumOffsetSnapshotValues {
    param(
        [Parameter(Mandatory=$true)] [Int] $ExpectedCoreCount
    )
    ...
    try {
        $raw = Get-Content -LiteralPath $coreOffsetSnapshotFile -Raw -ErrorAction Stop
        ...
        if (!$snapshot -or !$snapshot.offsets) {
            return $null
        }

        $offsets = @($snapshot.offsets | ForEach-Object { [Int] $_ })

        if ($offsets.Count -ne $ExpectedCoreCount) {
            Write-DebugText('The Fiskum IT offset snapshot has ' + $offsets.Count + ' values, but ' + $ExpectedCoreCount + ' cores were expected - ignoring it')
            return $null
        }
        ...
        Write-DebugText('Could not read the Fiskum IT offset snapshot file: ' + $_)
        return $null
    }
}
...
function Initialize-AutomaticTestMode {
    ...
    # Fiskum IT: prefer a valid offset snapshot from a previous run over whatever
    # "startValues" says below, regardless of mode - see Get-FiskumOffsetSnapshotValues
    # for why this is needed (setVoltageOnlyForTestedCore means the live CPU state alone
    # is not a reliable source for "the values we already found")
    if (!$voltageStartValuesArray) {
        $voltageStartValuesArray = Get-FiskumOffsetSnapshotValues -ExpectedCoreCount $numPhysCores

        if ($voltageStartValuesArray) {
            Write-DebugText('Using the Fiskum IT offset snapshot file as the starting values:')
            Write-DebugText($voltageStartValuesArray)
        }
    }
    ...
    # Fiskum IT: start this run with a clean offset snapshot, a fresh one is written as soon as testing begins
    Remove-CoreOffsetSnapshot
...
function Set-CurveOptimizerValues {
    ...
    try {
        ...
        for ($numTry = 1; $numTry -le $maxTries; $numTry++) {
            ...
            if ($exitCode -ne 0) {
                # Fiskum IT fix: $msg must be initialized before the += below, otherwise this throws
                # "variable has not been set" under Set-StrictMode instead of the actual CLI error
                $msg = $null
...
function Test-AutomaticTestModeIncrease {
    ...
    if ($useAutomaticTestMode) {
        ...
        if ($oldValue -ge $maxValue) {
            ...
            # Fiskum IT: keep the offset snapshot in sync with the value we just changed
            Set-CoreOffsetSnapshot -ActiveCore $actualCoreNumber
...
<#
.DESCRIPTION
    Fiskum IT addition: the counterpart to Test-AutomaticTestModeIncrease for the "Decreasing" search direction.
    Used by the "Assistert undervolting" workflow: instead of starting at an aggressive value and backing off on
    error, this starts at the configured startValues (typically 0) and makes the Curve Optimizer / voltage offset
    value MORE aggressive every time the core passes its test without an error, repeating the same core.
    Once the core does throw an error, the existing Test-AutomaticTestModeIncrease (called from the normal error
    handling path) steps the value back up by one incrementBy and - with repeatCoreOnError=0 - locks it there and
    moves on to the next core. This function is a no-op unless searchDirection is set to "Decreasing".
.PARAMETER actualCoreNumber
    [Int] The core that just passed its test
.PARAMETER coreTestOrderArray
    [System.Collections.ArrayList] The array of the cores being tested. We're modifying this to repeat the core
.PARAMETER coreIndex
    [Ref] The current core index. We're modifying this to repeat the core
#>
function Test-AutomaticTestModeDecrease {
    param (
        [Parameter(Mandatory=$true)] [Int] $actualCoreNumber,
        [Parameter(Mandatory=$true)][AllowEmptyCollection()] [System.Collections.ArrayList] $coreTestOrderArray,
        [Parameter(Mandatory=$true)] $coreIndex
    )

    if (!$useAutomaticTestMode -or $settings.AutomaticTestMode.searchDirection -ne 'Decreasing') {
        return
    }

    $incrementBy         = [Int] $settings['AutomaticTestMode']['incrementBy']
    # Fiskum IT: safety floor, see the "minValue" doc in default.config.ini. Falls back to no floor
    # (-2147483648) if the setting is somehow missing, e.g. an older config file from before this addition
    $minValue            = $(if ($null -ne $settings['AutomaticTestMode']['minValue']) { [Int] $settings['AutomaticTestMode']['minValue'] } else { [Int]::MinValue })
    $oldValue            = $(if ($useCurveOptimizer) { [Int] $voltageCurrentValues[$actualCoreNumber] } elseif ($useIntelVoltageAdjustment) { [Int] $voltageCurrentValues[0] })
    ...
    # Fiskum IT: do not push the value past the configured safety floor, even if the core never
    # throws an error - lock it at its current value and move on to the next core instead
    if ($oldValue -le $minValue) {
        Write-Text('')
        Write-ColorText('Automatic Test Mode (Decreasing / Assistert undervolting) is enabled') Yellow
        Write-ColorText('This core passed without error, but the ' + $autoModeDescription + ' value' + $forCore + ' is already at the configured safety floor of ' + $minValue + ' - locking it here and moving on') DarkYellow
        Write-Text('')
        return
    }
    ...
    Write-Text('')
    Write-ColorText('Automatic Test Mode (Decreasing / Assistert undervolting) is enabled') Yellow
    Write-ColorText('This core passed without error, decreasing the ' + $autoModeDescription + ' value' + $forCore + ' from ' + $oldValueStr + ' to ' + $newValueStr) Yellow
    ...
    # Apply the new, more aggressive values
    Set-NewVoltageValues

    # Fiskum IT: persist the always-on snapshot so the Manager can show/resume the live search state
    Set-CoreOffsetSnapshot -ActiveCore $actualCoreNumber

    if ($canUseWindowsEventLog) {
        Write-AppEventLog -type 'core_co_value' -infoString1 $actualCoreNumber -infoString2 $oldValue -infoString3 $newValue
    }

    # Repeat the same core to test the new, more aggressive value
    Write-DebugText('Core test order array before: ' + $coreTestOrderArray)
    [Void] $coreTestOrderArray.Insert(0, $actualCoreNumber)
    Write-DebugText('Core test order array after:  ' + $coreTestOrderArray)
...
"@)
    ...
    for ($iteration = 1; $iteration -le $settings.General.maxIterations; $iteration++) {
        ...
        :LoopCoreRunner for ($coreIndex = 0; $coreIndex -lt $numAvailableCores; $coreIndex++) {
            ...
            # Fiskum IT: always keep the offset snapshot current, regardless of the resume setting
            Set-CoreOffsetSnapshot -ActiveCore $actualCoreNumber
                ...
                if ($useAutomaticRuntimePerCore -and $isPrime95) {
                    :LoopCheckForAutomaticRuntime while ($true) {
                        ...
                        if ($proceedToNextCore) {
                            ...
                            # Fiskum IT: "Assistert undervolting" - this core passed, try a more aggressive value next
                            Test-AutomaticTestModeDecrease -actualCoreNumber $actualCoreNumber -coreTestOrderArray $coreTestOrderArray -coreIndex ([Ref] $coreIndex)
...
# Maybe the user wants to check the stress test program output after terminating the script
    ...
    # Fiskum IT: do NOT remove the offset snapshot here anymore - it must survive a clean exit
    # so that the NEXT test (the next Vanlig stabilitetstest in the sequence, or the switch from
    # Assistert undervolting to Vanlig stabilitetstest) can seed itself from it, see
    # Get-FiskumOffsetSnapshotValues. The Manager never relies on this file's mere presence to
    # detect a crash anyway - that uses state.json's own status field - so nothing depended on
    # deleting it here. It is still cleared at the START of the next Automatic Test Mode run
    # (see Initialize-AutomaticTestMode) and on a fatal error (see Exit-WithFatalError)
