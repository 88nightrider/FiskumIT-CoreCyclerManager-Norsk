...
- **IntelVoltageControl** (bundlet i `CoreCycler/tools/`) er skrevet av [jamestut](https://github.com/jamestut/IntelVoltageControl) (MIT, se [`IntelVoltageControl-LICENSE-MIT.txt`](CoreCycler/tools/IntelVoltageControl/IntelVoltageControl-LICENSE-MIT.txt)). Bruker fra v0.8.7.11 [PawnIO](https://github.com/namazso/PawnIO) i stedet for WinRing0 for MSR-tilgang (WinRing0 flagges av antivirus siden mars 2025, CVE-2020-14979 - traff alle store WinRing0-baserte verktøy). IOCTL/mailbox-kommunikasjonen mot MSR 0x150 er bekreftet korrekt (`show`/`set` gir riktig svar, ingen feil) på ekte Intel-maskinvare - men selve spenningsendringen kunne IKKE bekreftes ende-til-ende på testmaskinen, fordi BIOS-en der har Plundervolt-låsen ("CFG Lock"/"Overclocking Lock") aktiv - samme generelle BIOS-/firmware-begrensning som allerede er omtalt i [`Vedlikehold-Prosessorstotte.md`](Vedlikehold-Prosessorstotte.md) ("kan være BIOS-blokkert... gjelder for ALLE Intel-generasjoner"). Dette er IKKE en feil i PawnIO-porteringen - WinRing0 ville møtt nøyaktig samme lås, siden begge bare utfører samme WRMSR-instruksjon. Kildekoden og full byggedokumentasjon for patchen ligger i [`IntelVoltageControlPawnIO-patch/`](IntelVoltageControlPawnIO-patch/BYGG-OG-PATCH-README.txt) i repo-roten (bevisst utenfor `CoreCycler/`/`Manager/`, slik at den ikke følger med i selve programvareinstallasjonen). PawnIO-driveren (bundlet i `CoreCycler/tools/PawnIO/`) krever en engangs, manuell installasjon - Installer.bat starter denne automatisk for Intel-CPUer.
- **TM5/TestMem5** (bundlet i `CoreCycler/tools/TestMem5/`) brukes for RAM-/Infinity Fabric-isolasjonstesting (se "Kjør TM5..." under Verktøy i UI-et) - har **ingen formell, bekreftet lisens** (uformell community-freeware), se [`LISENS-OG-OPPRINNELSE.txt`](CoreCycler/tools/TestMem5/LISENS-OG-OPPRINNELSE.txt) i samme mappe for en åpen forklaring av denne avgjørelsen.
